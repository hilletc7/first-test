...
dat1 <- read.csv(url("https://covid.ourworldindata.org/data/owid-covid-data.csv"))
...
covid.total <- function(x, dat=dat1){
  x <- x
  ...
  if(x %in% sdat$dat.location){
    ...
    cat("\nInput not recognized.\n\nUse countries() to view a list of valid inputs\n\n")
  }
}

covid.new <- function(x, dat=dat1){
  x <- x
  colors <- c("cases"="red", "deaths"="black")
  sdate <- as.Date(dat$date)
  dat <- cbind(dat, sdate)
...
covid.domestic.total <- function(x, dat=dat2){
  ...
  animate(p, renderer = gifski_renderer(loop=F))}
    ...
    cat("\nInput not recognized.\n\nUse states() to view a list of valid inputs\n\n")
  }
}
